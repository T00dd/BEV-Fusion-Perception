...
from scene_lights import SceneLights


def condition_is_night(condition):
    # Night if the sun is at/below the horizon. Works for both hand-written
    # conditions (sun_altitude_angle in weather) and CARLA presets named *Night.
    name = str(condition.get("name", "")).lower()
    if "night" in name:
        return True
    sun = condition.get("weather", {}).get("sun_altitude_angle", 90)
    try:
        return float(sun) <= 0.0
...
def record_scene(client, world, scene_id, scene_dir, condition, cfg, logger,
    ...
    # Brighten the controllable map light(s) for night presets, restore by day.
    scene_lights = SceneLights(world)
    is_night = condition_is_night(condition)
    scene_lights.set_night(is_night)
    if is_night:
        logger.info(f"[{scene_id}] night preset -> map lights brightened")
