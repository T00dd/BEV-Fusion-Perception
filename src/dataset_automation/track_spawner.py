...
sys.path.insert(0, str(Path(__file__).parent.parent / "lib/track-generator"))
from track_generator import Mode, SimType, TrackGenerator

# Change these coordinates to where you want the track to be spawned
...
# fallback for problems on the config files
def load_spawner_config():
    ...
    try:
        ...
        print(f"[track_spawner] {MAIN_CONFIG_PATH} not found, using defaults.")
    return cfg
...
# Shrink the track scale if needed to ensure all cones fit within the map bounds
def fit_scale_to_map(cones, start_x, start_y, scale, world):
    ...
    # Largest absolute offset from the track origin in each axis
    max_off_x = max(abs(c[0]) for c in all_cones)
    max_off_y = max(abs(c[1]) for c in all_cones)
 
    # Headroom from the start point to each map edge
    ...
    # Max scale that keeps every cone inside the box on both axes
...
def spawn_track(cones, start_x, start_y, scale, bp_left_id, bp_right_id):
    ...
    model_cones_left = blueprints.filter("static.prop.blue_cone")[0]
    model_cones_right = blueprints.filter("static.prop.yellow_cone")[0]

    def highlight(location, label, color):
        world.debug.draw_point(location, size=0.35, color=color, life_time=60.0)
    ...
    for i, cone_left in enumerate(cones_left, start=1):
    ...
    for i, cone_right in enumerate(cones_right, start=1):
        location = carla.Location(
...
def generate_and_spawn_track():
    ...
    # load spawner config with defaults
    ...
    # load configuration
    config_path = Path(__file__).parent.parent / "lib/track-generator-sw/config.yaml"
    ...
    # attempt to generate and spawn track, with retries on failure (e.g. track out of bounds)
    max_attempts = 10
    attempt = 0
    success = False
    cones = None
    ...
    while attempt < max_attempts and not success:
        try:
            ...
            spawn_track(cones, start_x, start_y, scale, bp_left_id, bp_right_id)
            print("Track spawned!")
